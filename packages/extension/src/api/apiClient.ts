...
/**
 * Manages a typed OpenChoreo API client lifecycle.
 *
 * Recreates the client whenever the token or base URL changes
...
export class ApiClientManager {
  ...
  async getClient(): Promise<OpenChoreoClient | undefined> {
    ...
    const baseUrl = `${session.controlPlaneUrl}/api/v1`;
